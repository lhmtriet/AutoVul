...
def evaluate(clf, x_train, y_train, x_val, y_val, x_test, y_test, clf_settings, outpath, write=True):
	...
	if not os.path.exists(outpath):
		...
		outfile.write("repo_name,data_type,alpha,partition,feature,classifier,parameters,"
...
def vstack_features(r):
	global combined_features
	if not isinstance(r, float):
		if combined_features == None:
			combined_features = r
		else:
			combined_features = vstack([combined_features, r])


def main(repo_name, data_type, feature, model_name, alpha):

	global combined_features

	alpha = float(alpha)
	print(repo_name, data_type, feature, model_name, alpha)
	...
	result_file = f'{result_folder}{repo_name}_{data_type}_{feature}_{model_name}_{alpha}_results.csv'
	...
	for index in range(len(folds)):
		...
		# Cleaning data step
		print('Cleaning D2A dataset')
		
		train_df = df[(df[key_col].isin(train_entries))]
		train_df_reg = train_df[train_df['is_vuln_file'] == -1].copy()
		x_train_reg_init = train_df_reg[data_col].values
		y_train_reg = train_df_reg[label_col].values
		
		train_df_d2a = df[(~((df['is_vuln'] == 1) & (df['is_vuln_file'] == -1)))]
		train_df_d2a = train_df_d2a.loc[train_df_d2a['is_vuln'] == 1].copy()
		x_train_d2a_init = train_df_d2a[data_col].values
		y_train_d2a = train_df_d2a[label_col].values

		print('Initial d2a size:', len(train_df_d2a), Counter(y_train_d2a))
		
		# Validation & Testing sets, but not used for the cleaning
		...
		x_train_reg, x_train_d2a, _ = generate_features(x_train_reg_init, x_train_d2a_init, [], feature)
		
		print('Feature execution time prior cleaning:', time.time() - feature_time, 's.')
		
		if isinstance(x_train_reg, scipy.sparse._csr.csr_matrix):

			print('Converting from csr_matrix to numpy type')
			centroid_time = time.time()

			### Updated code to handle sparse data
			train_tmp_df = pd.DataFrame(x_train_reg, columns=['feature'])
			d2a_tmp_df = pd.DataFrame(x_train_d2a, columns=['feature'])

			print('Initial d2a size', len(d2a_tmp_df))

			vuln_indices = train_df_reg.loc[train_df_reg['is_vuln'] == 1].reset_index(drop=True).index.values.astype(int)
			train_tmp_df.iloc[vuln_indices,:]['feature'].map(lambda r: vstack_features(r))
			pos_centroid = combined_features.mean(axis=0)
			pos_centroid = np.asarray(pos_centroid).reshape(-1)
			print('Pos centroid', pos_centroid.shape)

			combined_features = None

			non_vuln_indices = train_df_reg.loc[train_df_reg['is_vuln'] == 0].reset_index(drop=True).index.values.astype(int)
			train_tmp_df.iloc[non_vuln_indices,:]['feature'].map(lambda r: vstack_features(r))
			neg_centroid = combined_features.mean(axis=0)
			neg_centroid = np.asarray(neg_centroid).reshape(-1)

			print('Neg centroid', neg_centroid.shape)

			combined_features = None
			
			d2a_tmp_df['selected'] = d2a_tmp_df['feature'].map(lambda x: distance.cosine(x.toarray().ravel(), pos_centroid) < alpha * distance.cosine(x.toarray().ravel(), neg_centroid))
			selected_indices = d2a_tmp_df.loc[d2a_tmp_df['selected'] == True].index.values.astype(int)
			
			train_df_d2a_tmp = train_df_d2a.iloc[selected_indices,:].copy()

			print('Centroid computing time:', time.time() - centroid_time, 's.')

		else:

			centroid_time = time.time()
			
			train_df_reg['feature'] = x_train_reg.tolist()
			train_df_d2a['feature'] = x_train_d2a.tolist()

			print('Before removing', len(train_df_reg))
			train_df_reg = train_df_reg.loc[train_df_reg['feature'].notnull()]
			train_df_d2a = train_df_d2a.loc[train_df_d2a['feature'].notnull()]

			print('After removing', len(train_df_reg))

			pos_centroid = np.mean(train_df_reg.loc[train_df_reg['is_vuln'] == 1]['feature'].tolist(), axis=0)
			neg_centroid = np.mean(train_df_reg.loc[train_df_reg['is_vuln'] == 0]['feature'].tolist(), axis=0)

			train_df_d2a['selected'] = train_df_d2a['feature'].map(lambda x: distance.cosine(x, pos_centroid) < alpha * distance.cosine(x, neg_centroid))
		
			train_df_d2a_tmp = train_df_d2a.loc[train_df_d2a['selected'] == True].copy()
			
			print('Centroid computing time:', time.time() - centroid_time, 's.')

		if len(train_df_d2a_tmp) == 0:
			print('No labels found at', alpha)
			continue

		print('Size of D2A', len(train_df_d2a_tmp))

		if data_type == 'd2a':
			train_df_new = df[(df[key_col].isin(train_entries))]
			train_df_new = train_df_new[(~((train_df_new['is_vuln'] == 1) & (train_df_new['is_vuln_file'] == -1)))]
			train_df_new = train_df_new.loc[train_df_new['is_vuln'] == 0]
			train_df_new = pd.concat([train_df_new, train_df_d2a_tmp], ignore_index=True)
		elif data_type == 'both':
			train_df_new = pd.concat([train_df_reg, train_df_d2a_tmp], ignore_index=True)
		
		x_train = train_df_new[data_col].values
		y_train = train_df_new[label_col].values
		
		print(fold, len(x_train), Counter(train_df_new['is_vuln_file']), Counter(y_train), Counter(y_val), Counter(y_test))

		feature_time = time.time()

		x_train, x_val_feature, x_test_feature = generate_features(x_train, x_val, x_test, feature)
		
		print('Feature Execution time after cleaning:', time.time() - feature_time, 's.')
		...
		for parameters in param_set:
			...
			clf_settings = f"{repo_name},{data_type},{alpha},{fold},{feature},{model_name}," \
						   f"{'-'.join(parameters)}"
			print(clf_settings)
			multiclass = False if len(np.unique(y_train_reg)) else True

			model_time = time.time()
			
			# Get and evaluate the classifier
			clf = get_classifier(repo_name, model_name, multiclass, len(np.unique(y_train_reg)), dual, *parameters)

			evaluate(clf, x_train, y_train, x_val_feature, y_val, x_test_feature, y_test,
					 clf_settings, result_file, write=True)

			print('Model Execution time after cleaning:', time.time() - model_time, 's.')
			print('#' * 30)

	print('Execution time:', time.time() - start_time, 's.')

	avg_df = compute_average_results(result_file)
	avg_df.to_csv(f'{result_folder}avg_results_{repo_name}_{data_type}_{feature}_{model_name}_{alpha}.csv', index=False)
	best_results = find_best_results(avg_df)
	print('Best results:', best_results.values)
	best_results.to_csv(f'{result_folder}best_results_{repo_name}_{data_type}_{feature}_{model_name}_{alpha}.csv', index=False)
...
if __name__ == '__main__':
	...
	main(repo_name=sys.argv[1], data_type=sys.argv[2], feature=sys.argv[3], model_name=sys.argv[4], alpha=sys.argv[5])
