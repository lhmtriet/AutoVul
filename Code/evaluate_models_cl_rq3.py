...
def evaluate(clf, x_train, y_train, x_val, y_val, x_test, y_test, clf_settings, outpath, write=True):
	...
	if not os.path.exists(outpath):
		...
		outfile.write("repo_name,data_type,threshold,partition,feature,classifier,parameters,"
...
def main(repo_name, data_type, feature, model_name, threshold):

	threshold = float(threshold)
	print(repo_name, data_type, feature, model_name, threshold)
	...
	result_file = f'{result_folder}{repo_name}_{data_type}_{feature}_{model_name}_{threshold}_results.csv'
	...
	for index in range(len(folds)):
		...
		x_train_reg = train_df_reg[data_col].values
		...
		x_train_d2a = train_df_d2a[data_col].values
		...
		val_df_tmp = val_df.loc[val_df['is_vuln'] == 0].copy()
		x_val_tmp = val_df_tmp[data_col].values
		y_val_tmp = val_df_tmp[label_col].values
		
		###########
		
		feature_time = time.time()
		
		x_train_reg, x_train_d2a, x_val_tmp = generate_features(x_train_reg, x_train_d2a, x_val_tmp, feature)
		...
		for parameters in param_set:
			...
			train_df_d2a_tmp = train_df_d2a.copy()

			model_time = time.time()

			clf_settings = f"{repo_name},{data_type},{threshold},{fold},{feature},{model_name}," \
						   f"{'-'.join(parameters)}"
			print(clf_settings)
			multiclass = False if len(np.unique(y_train_reg)) else True

			n_rows, n_cols = x_train_reg.shape
			if n_rows > n_cols:
				dual = False
			else:
				dual = True
			...
			if model_name == 'svm':
				print('Calibrating svm classifier to obtain predicted probabilities')
				clf = CalibratedClassifierCV(base_estimator=clf, cv='prefit')
				clf.fit(x_train_reg, y_train_reg)
			
			print('Model Execution time prior cleaning:', time.time() - model_time, 's.')
			
			y_comb = np.r_[y_train_d2a, y_val_tmp]
			
			y_pred_d2a_probs = clf.predict_proba(x_train_d2a)
			y_pred_val_probs = clf.predict_proba(x_val_tmp)
			
			y_pred_comb = np.r_[y_pred_d2a_probs, y_pred_val_probs]
			
			d2a_label_issues = CleanLearning(clf, seed=42).find_label_issues(labels=y_comb, pred_probs=y_pred_comb)
			
			d2a_label_issues = d2a_label_issues.loc[d2a_label_issues['given_label'] == 1]
			d2a_label_issues['is_label_issue'] = d2a_label_issues['is_label_issue'].astype(bool)
			
			d2a_label_issues.reset_index(drop=True, inplace=True)
			train_df_d2a_tmp.reset_index(drop=True, inplace=True)
			
			train_df_d2a_tmp = train_df_d2a_tmp.assign(is_label_issue=d2a_label_issues['is_label_issue'])
			train_df_d2a_tmp = train_df_d2a_tmp.assign(label_quality=d2a_label_issues['label_quality'])
			
			original_len = len(train_df_d2a_tmp)
			
			train_df_d2a_tmp = train_df_d2a_tmp.loc[(train_df_d2a_tmp['is_label_issue'] == False) & (train_df_d2a_tmp['label_quality'] >= threshold)]
			
			if len(train_df_d2a_tmp) == 0:
				print('No labels found at', threshold)
				continue
	...
	avg_df.to_csv(f'{result_folder}avg_results_{repo_name}_{data_type}_{feature}_{model_name}_{threshold}.csv', index=False)
	best_results = find_best_results(avg_df)
	print('Best results:', best_results.values)
	best_results.to_csv(f'{result_folder}best_results_{repo_name}_{data_type}_{feature}_{model_name}_{threshold}.csv', index=False)
...
if __name__ == '__main__':
	...
	main(repo_name=sys.argv[1], data_type=sys.argv[2], feature=sys.argv[3], model_name=sys.argv[4], threshold=sys.argv[5])
